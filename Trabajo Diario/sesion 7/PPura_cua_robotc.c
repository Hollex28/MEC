...
task main()
{
  ...
  float V = 0.300; //velocidad avance robot
  ...
  float Ts = 0.050; //Periodo muestreo

  long current_point = 0;
  long num_points = 0;
  long j = 0;

  float saturation_factor = 1;
  float acumulator;
  ...
  //generación trayectoria cuadrada
  j=0;
  acumulator = 0;
  for (i=0; i<=50; i++) {
    t[j].x = center_x + acumulator;
    t[j].y = center_y + 0;
    acumulator += 0.020;
    j++;
  }
  ...
  for (i=1; i<=50; i++) {
    t[j].x = t[50].x;
    t[j].y = t[50].y + acumulator;
    acumulator += 0.020;
    j++;
  }
  acumulator = 0.020;
  for (i=1; i<=50; i++) {
    t[j].x = t[100].x - acumulator;
    t[j].y = t[100].y;
    acumulator += 0.020;
    j++;
  }
  ...
  while(indice < 600) //podría ser infinito while(1), pero se deja en 600 iteraciones

  {
    ...
    // calculamo velocidad lineal del robot
    velLin_robot=

    // calculo la velocidad angular del robot
    velAng_robot=

    // calculo la posicion X-Y y la orientacion del robot
    ...
    //Reset distancia minima a un valor lo suficientemente grande
    min_dist = 9999999999;
    ...
    //Calculation of the robot inverse kinematics
